...
- **Routing eval, five behavioral categories** (`evals/`) — a follow-up critique of this eval's own first draft correctly identified that "expected file exists" alone rewards over-routing and under-routing as long as *a* file gets picked. The case set now splits into `basic-routing` (20 cases, one clear answer), `no-specialist` (5 cases — trivial tasks that should load zero specialists, Tier 0), `multi-domain` (3 cases — tasks that genuinely need Tier 2/3, checks every load-bearing specialist gets named, not just one), `ambiguous` (3 cases — tasks with several defensible routes, so a reasonable alternate answer doesn't count as a failure), and `conflict-precedence` (3 cases — two loaded specialists disagree, e.g. Debugging vs. Security over printing env vars; checks the model's first concrete action contains no forbidden action, proving the precedence order actually changes behavior and isn't just documented). 34 cases total. Each category's static check was deliberately broken, confirmed to fail, then reverted, following the same verification discipline as the deterministic-validation skills.
